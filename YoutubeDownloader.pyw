...
class DownloaderApp(ctk.CTk):
    def __init__(self):
        super().__init__()
        self.title("neznaamey's youtube downloader")
        ...
        try:
            ...
            try:
                with yt_dlp.YoutubeDL({'outtmpl': os.path.join(self.download_path, '%(title)s.%(ext)s'), 'logger': QuietLogger()}) as ydl:
                    ydl.download([url])
                messagebox.showinfo("Hotovo", "Staženo v nejlepší dostupné kvalitě.")
                self.progress_bar.set(0)
            except:
                messagebox.showerror("Chyba", "Stahování selhalo.")
